####################################################################################################
####### This is file "recipient.R"
####### This version: 2014-02-04
####### I run the robustness checks for the synthetic control analyses of the recipients
####################################################################################################
...
# Show countries that are available for Synthetic Recipients

print(country <- sort(unique(scdata$country[scdata$ccode %in% contr])))

###### Generate Synth object (debt/gdp ratio)
sdata <- dataprep(foo = scdata[scdata$ccode %in% contr | scdata$country == "recipient countries",],
		predictors = pred,
		dependent = names(scdata[6]),
		unit.variable = "ccode",
		time.variable = "Year", 
		treatment.identifier = 2222,
		controls.identifier = contr, 
		time.predictors.prior = c(1983:1997),
...
path.plot(synth.res = synth.out, 
	...
	Xlab="Year",
  Legend=c("Recipient Countries","Synthetic Recipient Countries"),
