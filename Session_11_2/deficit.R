...
######################################################################
####### This is file "deficit.R"
####### This version: 2014-02-04
####### I run the Synthetic control analyses for the deficit
######################################################################
...
contr <- setdiff(contr, c(350,1111,2222,70,155,225,269,290,310,316,317,349,355,360,366,666,732,920,sort(unique(scdata$ccode[scdata$eu==0 & scdata$oecd==0]))))
...
#### Generate Synth object (deltadebt)
sdata <- dataprep(foo = scdata,
		predictors = pred,
		dependent = "primbaltot2",
		unit.variable = "ccode",
		time.variable = "Year", 
...
gaps.plot(synth.res = synth.out, dataprep.res = sdata,
	Ylab= "Gap in Deficit (percentage points, 1983-2010)", Xlab="Year",
