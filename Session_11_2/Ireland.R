...
# Show countries that are available for Synthetic Ireland
print(country <- sort(unique(scdata$country[scdata$ccode %in% contr])))



###### Generate Synth object (debt/gdp ratio)
sdata <- dataprep(foo = scdata[scdata$ccode %in% contr | scdata$country == "Ireland",],
		predictors = pred,
		dependent = names(scdata[6]),
		unit.variable = "ccode",
		time.variable = "Year", 
		treatment.identifier =  205,
		controls.identifier = contr, 
		time.predictors.prior = c(1995:1997),
...
path.plot(synth.res = synth.out, 
	...
	Xlab="Year",
	Legend=c("Ireland","Synthetic Ireland"),
