#########################################################################################
####### This is file "Italy.R"
####### Last change: 2014-02-04
####### I run the robustness checks for the synthetic control analyses of Italy
#########################################################################################
...
#### Show countries available for synthetic Italy
print(country <- sort(unique(scdata$country[scdata$ccode %in% contr])))

###### Generate Synth object (debt/gdp ratio)

sdata <- dataprep(foo = scdata[scdata$ccode %in% contr | scdata$country == "Italy",],
		predictors = pred,
		dependent = names(scdata[6]),
		unit.variable = "ccode",
		time.variable = "Year", 
		treatment.identifier =  325,
